...
//  首頁介面
//
//  Created by 鄒家禾 on 2017/5/17.
//  Copyright © 2017年 鄒家禾. All rights reserved.
//
import UIKit
import Foundation

class mySocialPic: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
    ...
    @IBAction func goDetail(_ sender: UIButton) {
        self.performSegue(withIdentifier: "goSocial", sender: sender.tag)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        controller.picname = list[tag]
        controller.detail = namelist[tag]
    ...
    func call(){
        ...
        var request = URLRequest(url: URL(string: URL_SAVE_TEAM)!)
        request.httpMethod = "POST"
        let postString = "uid="+id+"&class=social";
        //送資料到伺服器
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            let responseString = String(data: data, encoding: .utf8)
            print("進入社群圖庫 = \(String(describing: responseString))")
    ...
    func showitem(item:String){
        ...
        for i in 0 ..< clotheArray.count/2{
            list.append(clotheArray[2*i])
        }
        //衣服名稱加入namelist
        
        for i in 0 ..< clotheArray.count/2{
            namelist.append(clotheArray[2*i+1])
        }
        //衣服圖片加入list
        self.refresh()
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cellForSocial", for: indexPath) as! cellForSocial
        //let closetItem = cell.closetItem as! UIButton
        //let itemName = cell.itemName as! UILabel
        cell.socialpic.tag = indexPath.row
        DispatchQueue.main.async {
        cell.socialpic.downloadedFrom(link: "http://140.119.19.18/imgup/"+self.list[indexPath.row])
        }
